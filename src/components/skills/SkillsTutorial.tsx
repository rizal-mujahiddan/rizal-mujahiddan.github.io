        ...
        <div className="mt-8 rounded-2xl border border-white/10 bg-white/3 p-1.5">
          <div className="flex gap-1">
            {(["hard", "soft"] as SkillType[]).map((t) => (
              ...
              >
                {t === "hard" ? "Hard Skills · Technical" : "Soft Skills · Human"}
              </button>
            ))}
          </div>
        </div>
    ...
    <button
      ...
      className={`rounded-full border px-4 py-2 text-sm font-medium transition ${
        active
          ...
          : "border-white/10 bg-white/300 text-slate-400 hover:border-white/25 hover:text-white"
      }`}
    >
      {label}
    </button>
  ...
  return (
    <article className="rounded-2xl border border-white/10 bg-white/4 p-6 transition hover:-translate-y-1 hover:border-cyan-300/30">
      <div className="flex items-start justify-between gap-4">
        ...
        <span className="rounded-full border border-cyan-300/25 bg-cyan-300/10 px-2.5 py-0.5 text-xs text-cyan-200">
          {skill.category}
        </span>
      </div>
